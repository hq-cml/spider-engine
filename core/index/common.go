...
// 索引类型说明
const (
	IDX_TYPE_STRING        = 101 //字符型索引[全词匹配，倒排，用于姓名等字段]
	IDX_TYPE_STRING_SEG    = 102 //字符型索引[切词匹配，倒排，用于全文索引]
	IDX_TYPE_STRING_LIST   = 103 //字符型索引[列表类型，倒排，分号切词]
	IDX_TYPE_STRING_SINGLE = 104 //字符型索引[单字切词，倒排，自然语言单个字母或者汉字]

	IDX_TYPE_NUMBER        = 111 //数字型索引，只支持整数，数字型索引只建立正排
	IDX_TYPE_DATE          = 115 //日期型索引 '2015-11-11 00:11:12'，日期型只建立正排，转成时间戳存储

	IDX_TYPE_PK            = 121 //主键类型，特殊，只有倒排，使用独立的B+树存储
	GATHER_TYPE            = 122 //汇总类型，倒排正排都需要[后续使用]

	IDX_ONLYSTORE          = 130 //只保存详情，不参与检索
)
