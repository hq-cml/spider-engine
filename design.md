...
### 共享内存mmap实现：
spider-engine中大量使用了mmap，封装了syscall的Mmap。mmap的好处是使得文件操作像操作内存一般方便，并能够由系统来保证一致性的同步。
设计了隐藏的header，用于保存实际的内存写入位置（因为映射的文件并不都是完整使用了的，可能使用一部分。所以为了保证再次加载之后能够继续写入，设计了这个隐藏Header保存实际的写入位置）
![Mmap](./img/spider-mmap.jpg)
